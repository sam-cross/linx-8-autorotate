...
int rotation_changed(){
	...
	if(accel_y < -accel_g) state = 2; // 0
	else if(accel_y > accel_g) state = 3; // 1
#if N_STATE == 4
	else if(accel_x > accel_g) state = 0; // 2
	else if(accel_x < -accel_g) state = 1; // 3
#endif

	fprintf(stderr, "\tCurrent state: %d\n\tNew state: %d", current_state, state);
